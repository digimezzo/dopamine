...
describe('ArtistService', () => {
    ...
    function createTrackArtistDatas(): ArtistData[] {
        ...
        trackArtistDatas.push(new ArtistData(';Aerosmith ft. Taylor Swift;'));

        return trackArtistDatas;
    }

    function createAlbumArtistDatas(): ArtistData[] {
        ...
        albumArtistDatas.push(new ArtistData(';Alanis Morissette feat. Daughter;'));

        return albumArtistDatas;
    }

    describe('constructor', () => {
        it('should create', () => {
            // Act
            const service: ArtistServiceBase = new ArtistService(translatorServiceMock.object, artistSplitter, trackRepositoryMock.object);
    ...
    describe('getArtists', () => {
        it('should get all track artists without duplicates when given ArtistType.trackArtists', () => {
            // Arrange
            const trackArtistDatas: ArtistData[] = createTrackArtistDatas();
            const albumArtistDatas: ArtistData[] = createAlbumArtistDatas();
            ...
            expect(artists[6].displayName).toEqual('Aerosmith ft. Taylor Swift');
        });
        ...
        it('should get all album artists without duplicates when given ArtistType.albumArtists', () => {
            ...
            expect(artists[7].displayName).toEqual('Alanis Morissette feat. Daughter');
        });
        ...
        it('should get all track and album artists without duplicates when given ArtistType.allArtists', () => {
            ...
            // Assert
            expect(artists.length).toEqual(11);
            expect(artists[0].displayName).toEqual('Aerosmith');
            expect(artists[1].displayName).toEqual('Alanis Morissette');
            expect(artists[2].displayName).toEqual('Bon Jovi');
            expect(artists[3].displayName).toEqual('Madonna');
            ...
            expect(artists[10].displayName).toEqual('Alanis Morissette feat. Daughter');
        });
    });
});
