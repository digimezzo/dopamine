...
describe('ListenbrainzProvider', () => {
    ...
    function createComponent(): ListenbrainzProvider {
        return new ListenbrainzProvider(
            listenbrainzApiMock.object,
            settingsStub,
            loggerMock.object,
        );
    }
    ...
    describe('signInStateChanged$', () => {
        it('should notify when signed in', async () => {
            ...
            listenbrainzApiMock.setup(x => x.getUsernameByToken('test-token'))
                .returns(() => Promise.resolve('test-user'));
    ...
    describe('updateNowPlayingAsync', () => {
        it('should update now playing when signed in', async () => {
            // Arrange
            settingsStub.listenbrainzToken = 'test-token';
            listenbrainzApiMock.setup((x) => x.updateNowPlayingAsync('test-token', 'test-artist', 'test-track', 'test-album'))
            ...
            // Assert
            listenbrainzApiMock.verify(x => x.updateNowPlayingAsync('test-token', 'test-artist', 'test-track', 'test-album'), Times.once());
        ...
        it('should not update now playing when not signed in', async () => {
            ...
            const provider = createComponent();
            provider.initialize();
            listenbrainzApiMock.setup((x) => x.updateNowPlayingAsync(It.isAny(), It.isAny(), It.isAny(), It.isAny()))
                .returns(() => Promise.resolve(true)).verifiable();
            const track: any = {
                artists: 'test-artist',
                title: 'test-track',
    ...
    describe('scrobbleAsync', () => {
        it('should scrobble when signed in', async () => {
            ...
            listenbrainzApiMock.setup((x) => x.scrobbleTrackAsync('test-token', 'test-artist', 'test-track', 'test-album', playbackStartTime))
                .returns(() => Promise.resolve(true)).verifiable();
            const provider = createComponent();
            provider.initialize();
            const track: any = {
            ...
            // Assert
            listenbrainzApiMock.verify(x => x.scrobbleTrackAsync('test-token', 'test-artist', 'test-track', 'test-album', playbackStartTime), Times.once());
            expect(result).toBe(true);
        });
        ...
        it('should not scrobble when not signed in', async () => {
            ...
            provider.initialize();
            const playbackStartTime: Date = new Date();
            listenbrainzApiMock.setup((x) => x.scrobbleTrackAsync(It.isAny(), It.isAny(), It.isAny(), It.isAny(), It.isAny()))
                .returns(() => Promise.resolve(true)).verifiable();
            const track: any = {
                artists: 'test-artist',
                title: 'test-track',
            ...
            // Assert
            listenbrainzApiMock.verify(x => x.scrobbleTrackAsync(It.isAny(), It.isAny(), It.isAny(), It.isAny(), It.isAny()), Times.never());
            expect(result).toBe(false);
        });
    });
