...
describe('AlbumArtworkIndexer', () => {
    describe('indexAlbumArtworkAsync', () => {
        ...
        it('Should not remove album artwork if there is no album data that needs indexing', async () => {
            ...
            // Assert
            albumArtworkRemoverMock.verify(x => x.tryRemoveAlbumArtwork(It.isAnyString()), Times.never());
        });
        ...
        it('Should remove album artwork if there is album data that needs indexing', async () => {
            ...
            // Assert
            albumArtworkRemoverMock.verify(x => x.tryRemoveAlbumArtwork(It.isAnyString()), Times.exactly(2));
            albumArtworkRemoverMock.verify(x => x.tryRemoveAlbumArtwork(albumData1.albumKey), Times.exactly(1));
            albumArtworkRemoverMock.verify(x => x.tryRemoveAlbumArtwork(albumData2.albumKey), Times.exactly(1));
        });
        ...
        it('Should not add album artwork if removing album artwork failed', async () => {
            ...
            trackRepositoryMock.setup(x => x.getAlbumDataThatNeedsIndexing()).returns(() => [albumData1, albumData2]);
            albumArtworkRemoverMock.setup(x => x.tryRemoveAlbumArtwork(It.isAnyString())).returns(() => false);
        ...
        it('Should add album artwork if removing album artwork succeeded', async () => {
            ...
            trackRepositoryMock.setup(x => x.getAlbumDataThatNeedsIndexing()).returns(() => [albumData1, albumData2]);
            albumArtworkRemoverMock.setup(x => x.tryRemoveAlbumArtwork(It.isAnyString())).returns(() => true);
